...
class HermiteKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.degree >= 1:
            cheby2[:, :, 1] = 2*x  # First-order polynomial: U_1(x) = 2x
        if self.degree >= 2:
            cheby2[:, :, 2] = 4 * x**2 - 2  # Second-order polynomial: U_2(x) = 4x^2 - 1
        if self.degree >= 3:
            cheby2[:, :, 3] = 8 * x**3 - 12*x  # Third-order polynomial: U_3(x) = 8x^3 - 4x
        if self.degree >= 4:
            cheby2[:, :, 4] = 16 * x**4 - 48 * x**2 + 12  # Fourth-order polynomial: U_4(x) = 16x^4 - 8x^2 + 1
...
class HermiteKAN(nn.Module):
    ...
    def __init__(self, network, degree):
        ...
        for i in range(len(network) - 1):
            self.layers.append(HermiteKANLayer(network[i], network[i + 1], degree))
...
class HermitePINN(nn.Module):
    ...
    def __init__(self, network, degree):
        ...
        """
        super(HermitePINN, self).__init__()
        # Degree of Chebyshev polynomials
        self.model = HermiteKAN(network, degree)  # Define the core ChebyKAN network
        self.network = network
        self.layers = nn.ModuleList()
