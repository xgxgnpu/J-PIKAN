...
class TaylorKANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.degree >= 2:
            cheby2[:, :, 2] = x**2  # Second-order polynomial: U_2(x) = 4x^2 - 1
        if self.degree >= 3:
            cheby2[:, :, 3] = x**3  # Third-order polynomial: U_3(x) = 8x^3 - 4x
        if self.degree >= 4:
            cheby2[:, :, 4] = x**4  # Fourth-order polynomial: U_4(x) = 16x^4 - 8x^2 + 1
...
class TaylorKAN(nn.Module):
    ...
    def __init__(self, network, degree):
        ...
        for i in range(len(network) - 1):
            self.layers.append(TaylorKANLayer(network[i], network[i + 1], degree))
...
class TaylorPINN(nn.Module):
    ...
    def __init__(self, network, degree):
        ...
        """
        super(TaylorPINN, self).__init__()
        # Degree of Chebyshev polynomials
        self.model = TaylorKAN(network, degree)  # Define the core ChebyKAN network
        self.network = network
        self.layers = nn.ModuleList()
